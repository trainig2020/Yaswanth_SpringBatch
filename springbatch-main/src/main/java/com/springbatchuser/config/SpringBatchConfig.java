...
public class SpringBatchConfig {
	...
	// @StepScope
	public Step step1() {
		return stepBuilderFactory.get("step1").<User, User>chunk(3).reader(multiResourceItemReader3()).processor(mProc)
				.writer(msdWrt).taskExecutor(taskExecutor2()).build();
	}

	/*
	 * @Bean
	 * 
	 * public Step step2() { return stepBuilderFactory.get("step").<User,
	 * User>chunk((3))
	 * 
	...
	public FlatFileItemReader<String> readerSql4() {
		...
		return new FlatFileItemReaderBuilder<String>().name("readerSql5").lineMapper(new PassThroughLineMapper())
				.build();
	}
...
//		Resource[] reso = null;
//		try {
//			reso = resolver.getResources("file:f:/sqlscripts/*.sql");
//
//		} catch (IOException e) {
//			// TODO Auto-generated catch block
//			e.printStackTrace();
...
//	}
	...
	public MultiResourceItemReader<String> multiResourceItemReaderSql4() {
		...
		try {
			...
			int ii = 0;
			Set<Resource> liss = new HashSet<>();
			for (Resource resourcee : reso) {

				if (resourcee.getFilename().contains("_")) {
					if (resourcee.getFilename().contains("_version1")) {

						System.out.println("resource names FIRST " + resourcee.getFilename());
						liss.add(resourcee);
						System.out.println("added to liist");
					}
				} else {
			...
			for (Resource resourcen : liss) {

				System.out.println("available lis files " + resourcen.getFilename());
			}

			System.out.println("length of rdf " + liss.size());

			Resource[] rdff1 = liss.stream().toArray(Resource[]::new);
			System.out.println("length of rdf1 " + rdff1.length);

			for (Resource resourc : rdff1) {
				System.out.println("resource name " + resourc.getFilename());
			}
			System.out.println("executes multi item reader");

			resourceItemReader4.setResources(rdff1);
			resourceItemReader4.setDelegate(readerSql4());
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
	...
	public FlatFileItemReader<String> readerSql3() {
		...
		System.out.println("reader4 reads value");
		return new FlatFileItemReaderBuilder<String>().name("reader4").lineMapper(new PassThroughLineMapper()).build();
	}
	...
	Connection getConnection() {
		...
		try {
			try {
				...
				con = DriverManager.getConnection("jdbc:mysql://localhost:3306/dg", "root", "yaswanth@123");
